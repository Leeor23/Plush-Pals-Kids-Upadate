...
function parseCSV(text: string): Item[] {
  ...
  for (let i=1;i<lines.length;i++){
    ...
    const s = String(obj.tieDye).toLowerCase(); obj.tieDye = (s==='true'||s==='yes'||s==='1')
...
export default function App(){
  ...
  // Inline Tie-Dye toggle
  async function toggleTieDye(it: Item, checked: boolean){
    const index = items.findIndex(x=>x===it); if(index<0) return
    const id = docIds[index]; if(!id) return
    const collPath = import.meta.env.VITE_FIRESTORE_COLLECTION || 'items'
  ...
  // Export / Import
  function exportCSV(){ const csv = toCSV(items); download(`inventory-${new Date().toISOString().slice(0,10)}.csv`, csv); }
    ...
    reader.onload=async ()=>{
      try{ const parsed=parseCSV(String(reader.result||'')); if(!parsed.length) return alert('No rows found in CSV');
        ...
        for (const row of normalized){
          const existingIndex = items.findIndex(x=>x.barcode===row.barcode)
          if (existingIndex>=0){
            const id = docIds[existingIndex]
            await setDoc(doc(fb.db, collPath, id), { ...row, _ts: serverTimestamp() })
          } else {
            await addDoc(collection(fb.db, collPath), { ...row, _ts: serverTimestamp() })
          }
        }
  ...
  async function handleImageFile(e: React.ChangeEvent<HTMLInputElement>){
    ...
    const r = sRef(fb.storage, `images/${safe}`)
  ...
  async function onSubmitForm(e: React.FormEvent<HTMLFormElement>){
    ...
    const payload: Item = {
      ...
      plush: titleCase(String(data.plush||'')),
      style: titleCase(String(data.style||'')),
      category: titleCase(String(data.category||'T-shirt')),
      color: titleCase(String(data.color||'')),
      nameDrop: titleCase(String(data.nameDrop||'')),
      quantity: Number(data.quantity||0),
      tieDye: data.tieDye==='on',
  ...
  return (<div className="min-h-screen bg-gradient-to-b from-gray-50 to-white text-gray-900">
    <header className="sticky top-0 z-30 bg-gradient-to-b from-white to-white/80 backdrop-blur border-b border-gray-200">
      <div className="mx-auto max-w-7xl px-4 py-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div><h1 className="text-2xl font-bold">Inventory — Firestore</h1><p className="text-sm text-gray-600">Realtime across devices. Anonymous auth; no login UI.</p></div>
        <div className="flex gap-2">
          <button className="rounded-xl bg-gray-900 text-white px-4 py-2 hover:bg-black" onClick={()=>setModalOpen(true) || openNew()}>Add Item</button>
          ...
          <input id="csvFile" ref={fileInputRef} type="file" accept=".csv,text/csv" className="hidden" onChange={importCSVHandler} />
        </div>
      </div>
    </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
        ...
        <div>
          ...
          <select value={filters.nameDrop} onChange={e=>setFilters({...filters, nameDrop:e.target.value})} className="w-full rounded-xl border px-3 py-2">
            <option>All</option>
            {distinct.nameDrop.map(v=><option key={String(v)}>{String(v)}</option>)}
          </select>
        </div>
        <div className="grid grid-cols-3 gap-3 md:grid-cols-3 md:col-span-1">
          ...
          <div><label className="block text-sm text-gray-600 mb-1">Plush</label>
            <select value={filters.plush} onChange={e=>setFilters({...filters, plush:e.target.value})} className="w-full rounded-xl border px-3 py-2"><option>All</option>{distinct.plush.map(v=><option key={String(v)}>{String(v)}</option>)}</select>
          </div>
          <div><label className="block text-sm text-gray-600 mb-1">Color</label>
            <select value={filters.color} onChange={e=>setFilters({...filters, color:e.target.value})} className="w-full rounded-xl border px-3 py-2"><option>All</option>{distinct.color.map(v=><option key={String(v)}>{String(v)}</option>)}</select>
          </div>
        </div>
        ...
        <div className="md:col-span-4 -mt-2 flex items-center gap-3">
          ...
          <div className="flex gap-2">
            {['All','Yes','No'].map(v => (
              <button key={v} onClick={()=>setFilters({...filters, tieDye:v as any})} className={classNames('px-3 py-1 rounded-full border', filters.tieDye===v ? 'bg-gray-900 text-white border-gray-900' : 'bg-white hover:bg-gray-50')}>{v}</button>
            ))}
          </div>
      ...
      <div className="overflow-auto rounded-2xl border border-gray-200 bg-white">
        <table className="min-w-full text-sm">
          ...
          <tbody>
            {filtered.map((it, idx)=>(<tr key={it.barcode} className="border-b last:border-0 hover:bg-gray-50/60">
              ...
              <td className="px-3 py-2">
                <label className="inline-flex items-center gap-2">
                  <input type="checkbox" checked={!!it.tieDye} onChange={(e)=>toggleTieDye(it, e.target.checked)} />
                  {it.tieDye?'Yes':'No'}
                </label>
              </td>
            ...
            {!filtered.length && (<tr><td colSpan={12} className="px-3 py-10 text-center text-gray-500">No items match your search/filters.</td></tr>)}
          </tbody>
        </table>
      </div>
    </section>
        ...
        <form onSubmit={onSubmitForm} className="space-y-3">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <div className="md:col-span-2">
              ...
              <p className="text-xs text-gray-500">Uploads go to Firebase Storage.</p>
            </div>
    ...
    <footer className="mx-auto max-w-7xl px-4 py-8 text-xs text-gray-500">
      <div>Live Firestore collection: <b>{import.meta.env.VITE_FIRESTORE_COLLECTION || 'items'}</b>. Fields: barcode, plush, style, category, color, nameDrop, quantity, tieDye, notes, imageUrl.</div>
    </footer>
  </div>)
}
